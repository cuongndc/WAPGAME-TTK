...
function getplayer($sid, $dblj){
    ...
    $stmt->bindColumn('uname',$player->uname);
    $stmt->bindColumn('sid',$player->sid);
    $stmt->bindColumn('uid',$player->uid);
    $stmt->bindColumn('ulv',$player->ulv);
    $stmt->bindColumn('uyxb',$player->uyxb);
    $stmt->bindColumn('uczb',$player->uczb);
    $stmt->bindColumn('uexp',$player->uexp);
    $stmt->bindColumn('uhp',$player->uhp);
    $stmt->bindColumn('umaxhp',$player->umaxhp);
    $stmt->bindColumn('ugj',$player->ugj);
    $stmt->bindColumn('ufy',$player->ufy);
    $stmt->bindColumn('ubj',$player->ubj);
    $stmt->bindColumn('uxx',$player->uxx);
    $stmt->bindColumn('uwx',$player->uwx);
    $stmt->bindColumn('usex',$player->usex);
    $stmt->bindColumn('vip',$player->vip);
    $stmt->bindColumn('nowmid',$player->nowmid);
    $stmt->bindColumn('endtime',$player->endtime);
    $stmt->bindColumn('tool1',$player->tool1);
    $stmt->bindColumn('tool2',$player->tool2);
    $stmt->bindColumn('tool3',$player->tool3);
    $stmt->bindColumn('tool4',$player->tool4);
    $stmt->bindColumn('tool5',$player->tool5);
    $stmt->bindColumn('tool6',$player->tool6);
    $stmt->bindColumn('sfxl',$player->sfxl);
    $stmt->bindColumn('xiuliantime',$player->xiuliantime);
    $stmt->bindColumn('yp1',$player->yp1);
    $stmt->bindColumn('yp2',$player->yp2);
    $stmt->bindColumn('yp3',$player->yp3);
    $stmt->bindColumn('jn1',$player->jn1);
    $stmt->bindColumn('jn2',$player->jn2);
    $stmt->bindColumn('jn3',$player->jn3);
    $stmt->bindColumn('cw',$player->cw);
    $stmt->bindColumn('sfzx',$player->sfzx);
    $stmt->bindColumn('ispvp',$player->ispvp);
...
function getyguaiwu($gyid,$dblj){//获取怪物库怪物
    ...
    if ($cxjg){
        ...
        $cxjg->fetch(\PDO::FETCH_ASSOC);
        $sjhp = rand(0,3) / 10;
        $guaiwu->ghp = $guaiwu->ghp + $guaiwu->ghp * $sjhp;
    }
    return $guaiwu;
}
...
function istupo($sid,$dblj){
    $game = new game();
    $player = $game->player_get_uinfo($sid,$dblj);
...
function addcwsx($sx,$gaibian,$cwid,$dblj){
    $sql = "update playerchongwu set $sx = $sx + '$gaibian' WHERE cwid='$cwid'";//增加宠物属性
    $ret = $dblj->exec($sql);
}
